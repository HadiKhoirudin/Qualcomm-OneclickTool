...
IF "%SelectedOperation%" == "-get_auth" (
    %emmcdl% -p %USBComPort% -f %Loader% -x auth.xml -memoryname %MemoryName% | findstr Failed
:: Done
echo. Done!
)
...
:: Erase FRP
    ...
    %emmcdl% -p %USBComPort% -f %Loader% -b persist.img -memoryname %MemoryName% >nul
